...
const ManagerUsers = () => {
  ...
  const handleBlockUser = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn khóa tài khoản người dùng này?")) {
      try {
        await axios.put(`${API_URL}/${id}`, { blocked: true }); // Chỉ cập nhật trường blocked
        fetchUsers();
      } catch (error) {
  ...
  const handleUnblockUser = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn mở khóa tài khoản người dùng này?")) {
      try {
        await axios.put(`${API_URL}/${id}`, { blocked: false }); // Chỉ cập nhật trường blocked
        fetchUsers();
      } catch (error) {
            ...
            render: (text, record) => (
              <>
                {record.blocked ? (
                  ...
                  <Button onClick={() => handleBlockUser(record.id)} style={{ marginRight: 10 }}>
                    Block
                  </Button>
                )}
              </>
